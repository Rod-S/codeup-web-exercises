...
//it's an array of objects with an array index representing each object
...
console.log("quote: "+firstQuote.quote);
console.log("author: "+firstQuote.author);
...
console.log("quote: "+lastQuote.quote);
console.log("author: "+lastQuote.author);
...
//violates DRY principle, tedious, not dynamic
...
function formatQuote(aphorism) {
	return aphorism.quote + " -" + aphorism.author
}
...
function everyOddQuote(aphorisms) {
	...
	for (var i=1; i<aphorisms.length; i+=2) {
		var oddQuote = {};
		oddQuote.id = i;
		oddQuote.quote = aphorisms[i].quote;
		oddQuote.author = aphorisms[i].author;
		oddQuote.rating = Math.floor(Math.random() * (5) + 1);
		oddQuoteArray.push(oddQuote);
	}
	return oddQuoteArray;
}